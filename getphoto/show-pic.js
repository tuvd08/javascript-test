var PIC = {
  ...
  markLayout : function () {
    ...
    wd = (wd > html.offsetHeight) ? wd : html.offsetHeight;
  ...
  displayElmPic: function (elm) {
    ...
    if(src.indexOf('google') > 0) {
        var k = FS.getKeyGl(src);
        src = src.replace(k, 's640');
    }
		img.src = src;
    PIC.img = img;
		PIC.id = 20;
		img.onload = PIC.IMGonload;
  ...
  IMGonload: function() {
    if (this != null) {
      var html = document.getElementsByTagName("html")[0];
      var cont = document.getElementById('contImg');
      cont.style.left = (html.offsetWidth - this.width) / 2 + "px";
      var sTop = (document.documentElement.scrollTop);
      var wd = document.documentElement.clientHeight;
      cont.style.top = (sTop + (wd - (this.height+20) )/2)  + "px";
      if (this.width > (html.offsetWidth - 200)) {
        this.width = (html.offsetWidth - 200);
      }
...
function initOnload() {
  ...
  var l = 0;
	var allprs = document.getElementById("renderData");
  var pics = allprs.getElementsByTagName("img");
