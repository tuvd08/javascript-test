...
/** ********************************************************************************* */
/**
 * This function aims is to configure the javascript environment according to
 * the browser in use
 * 
 * Common configuration are made first, then we detect the current browser and
 * according to the one in use, we call delegated methods such as this.initIE()
 * which will add some new configuration or overide the existing ones
 * 
 * In all cases the method createHttpRequest is acting as the usual
 * XMLHttpRequest in use in all AJAX calls
 */
function Browser() {
...
/**
 * Copyright (c) 2008, Yahoo! Inc. All rights reserved. Code licensed under the
 * BSD License: http://developer.yahoo.net/yui/license.txt version: 2.5.2
 */
Browser.prototype.detectBrowser = function() {
  ...
  /**
   * Gecko engine revision number. Will evaluate to 1 if Gecko is detected but
   * the revision could not be found. Other browsers will be 0. Example: 1.8
   * 
   * <pre>
  ...
  /**
   * AppleWebKit version. KHTML browsers that are not WebKit browsers will
   * evaluate to 1, other browsers 0. Example: 418.9.1
   * 
   * <pre>
  ...
  /**
   * The mobile property will be set to a string containing any relevant user
   * agent information when a modern mobile browser is detected. Currently
   * limited to Safari on the iPhone/iPod Touch, Nokia N-series devices with the
...
/**
 * Calls the functions in the onLoadCallback array, if they exist and clean the
 * array
 */
...
Browser.prototype.findMouseRelativeX = function(object, e, isRTL) {
  ...
  /*
   * posXObject is added more 3px on IE6 posXObject is double on IE7
   */
...
/**
 * Return the height of free space in the page if it is available. Otherwise,
 * returns a negative which is equal to the height of content not visible on the
 * screen.
 */
