...
DateTimePicker = function(calendarId) {
  ...
  this.pathResource = "../module/lang/";
  this.lang = "en";
  this.fistWeekDay = 0; // sunday: 0, monday: 1, tuesday: 2, wednesday: 3, thursday: 4, friday: 5, saturday: 6 
  this.load = true;
},

DateTimePicker.prototype.loadLang = function(text) {
  var picker = module.DateTimePicker;
  if (text) {
    var languages = eval(text);
    picker.months = languages[0];
    picker.weekdays = languages[1];
    picker.tooltip = languages[2];
  }
  picker.load = true;
};

DateTimePicker.prototype.setLang = function(lang) {
  var picker = module.DateTimePicker;
  try {
    if (!lang || picker.lang == lang)
      return;
    picker.lang = lang;
    picker.load = false;
    var url = picker.pathResource + picker.lang.toLowerCase() + ".js";
    eXo.core.Browser.ajaxAsyncGetContent(url, module.DateTimePicker.loadLang);
  } catch (e) {
    alert(e);
  }
};

DateTimePicker.prototype.getLang = function() {
  var picker = module.DateTimePicker;
  var day = picker.dateField.getAttribute('fistweekday');
  if (day)
    picker.fistWeekDay = day * 1 - 1; // attribute 'fistweekday' includes:
  // sunday: 1, monday: 2,..., saturday: 7
  var lang = picker.dateField.getAttribute('lang');
  module.DateTimePicker.setLang(lang);
};

DateTimePicker.prototype.init = function(field, isDisplayTime) {
  var picker = module.DateTimePicker;
  picker.isDisplayTime = isDisplayTime;
  if (picker.dateField) {
    picker.dateField.parentNode.style.position = '';
  }
  picker.dateField = field;
  module.DateTimePicker.getLang();
  module.DateTimePicker.showTimeout();
};

DateTimePicker.prototype.showTimeout = function() {
  var picker = module.DateTimePicker;
  if (picker.load === true) {
    if (!document.getElementById(picker.calendarId)) {
      picker.create();
    }
    picker.dateField.parentNode.insertBefore(document.getElementById(picker.calendarId), picker.dateField);
    picker.show();
  } else {
    setTimeout('module.DateTimePicker.showTimeout()', 300);
  }
};

DateTimePicker.prototype.create = function() {
  var clndr = document.createElement("div");
  clndr.id = this.calendarId;
  clndr.style.position = "absolute";
  if (document.all) {
    clndr.innerHTML = "<div class='UICalendarComponent'><iframe id='" + this.calendarId
...
DateTimePicker.prototype.show = function() {
  document.onmousedown = new Function('module.DateTimePicker.hide()');
  ...
  if (eXo.core.Browser.isIE6())
    clndr.getElementsByTagName("iframe")[0].style.height = drag.parentNode.offsetHeight + "px";
};
...
DateTimePicker.prototype.renderCalendar = function() {
  var dayOfMonth = 1;
  var validDay = 0;
  ...
  table += '   <tr>';
  table += '     <td class="MonthButton"><a class="PreviousMonth" title="' + this.tooltip[1] + '" href="javascript:module.DateTimePicker.changeMonth(-1);"></a></td>';
  table += '     <td class="YearButton"><a class="PreviousYear" title="' + this.tooltip[0] + '" href="javascript:module.DateTimePicker.changeYear(-1);"></a></td>';
  table += '     <td><font color="#f89302">' + this.months[this.currentDate.getMonth()] + '</font> - ' + this.currentDate.getFullYear() + '</td>';
  table += '     <td class="YearButton"><a class="NextYear" title="' + this.tooltip[3] + '" href="javascript:module.DateTimePicker.changeYear(1);"></a></td>';
  table += '     <td class="MonthButton"><a class="NextMonth" title="' + this.tooltip[2] + '" href="javascript:module.DateTimePicker.changeMonth(1);"></a></td>';
  table += '   </tr>';
  table += ' </table>';
  ...
  for ( var week = 0; week < 6; week++) {
    ...
    for ( var dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
      ...
      if (validDay) {
        if (dayOfMonth == this.selectedDate.getDate() && this.currentDate.getFullYear() == this.selectedDate.getFullYear()
        ...
        table = table + "<td><a class='" + clazz + "' href=\"javascript:module.DateTimePicker.setDate(" + this.currentDate.getFullYear() + "," + (this.currentDate.getMonth() + 1)
  ...
  if (this.isDisplayTime) {
    ...
    table += '     <div class="CalendarTimeBoxM"><span><input class="InputTime" size="2" maxlength="2" value="'
        ...
        + '" onkeyup="module.DateTimePicker.setHour(this)" >:<input size="2" class="InputTime" maxlength="2" value="'
        + ((this.currentDate.getMinutes()) > 9 ? this.currentDate.getMinutes() : "0" + this.currentDate.getMinutes())
        + '" onkeyup = "module.DateTimePicker.setMinus(this)">:<input size="2" class="InputTime" maxlength="2" value="'
        + ((this.currentDate.getSeconds()) > 9 ? this.currentDate.getSeconds() : "0" + this.currentDate.getSeconds())
        + '" onkeyup = "module.DateTimePicker.setSeconds(this)"></span></div>';
    table += '   </div>';
    table += ' </div>';
  }
  table += '</div>';
...
DateTimePicker.prototype.changeMonth = function(change) {
...
DateTimePicker.prototype.changeYear = function(change) {
...
DateTimePicker.prototype.setDate = function(year, month, day) {
  if (this.dateField) {
    if (month < 10)
      month = "0" + month;
    if (day < 10)
...
DateTimePicker.prototype.setSeconds = function(object) {
  if (this.dateField) {
    ...
    if (this.isDisplayTime)
      timeString = this.currentDay + "/" + this.currentMonth + "/" + this.currentYear + " " + timeString;
    this.dateField.value = timeString;
  }
  return;
};

DateTimePicker.prototype.setMinus = function(object) {
  if (this.dateField) {
...
DateTimePicker.prototype.setHour = function(object) {
  if (this.dateField) {
...
DateTimePicker.prototype.getDayOfWeek = function(year, month, day) {
  var date = new Date(year, month - 1, day);
  return date.getDay();
};

DateTimePicker.prototype.getDaysInMonth = function(year, month) {
  return [ 31, ((!(year % 4) && ((year % 100) || !(year % 400))) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 ][month];
};

module.DateTimePicker = new DateTimePicker('UICalendarControl');